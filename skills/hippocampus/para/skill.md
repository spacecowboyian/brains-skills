...
Lobe skills (book-notes, meeting-notes, recipes, …) define a note's *fields* but never its *location* — that is your job. When a lobe asks where one of its pages belongs, triage it by actionability like any content: tied to a deadline-bound goal → `projects/`; an ongoing responsibility → `areas/`; reference material → `resources/`; no longer active → `archives/`. Return the bucket path and have the lobe write it to the page's `filed_at`. You own only the location; the lobe owns everything inside the page.
