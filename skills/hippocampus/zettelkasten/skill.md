...
Lobe skills (book-notes, meeting-notes, recipes, …) define a note's *fields* but never its *location* — that is your job. When a lobe asks where one of its pages belongs, treat it as a literature or permanent note: it lives at `notes/<zettel_id>.md` like any zettel. Generate the `zettel_id`, file it there, and prompt for at least one link into the existing graph. Have the lobe write that path to the page's `filed_at`. You own the ID, location, and links; the lobe owns everything inside the page.
