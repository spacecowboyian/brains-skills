...
Read this file, then set up a recipes project in the user's Brains knowledge base using the template and instructions below. Create the project structure, initialize the instructions page, and offer to add the first recipe.

**Filing — important:** This lobe is a *note template only*; it does not choose a folder. Before creating pages, check whether a hippocampus skill (PARA, Johnny.Decimal, Zettelkasten, or Mind Palace) is active. If one is, ask it where each page belongs and record that path in the page's `filed_at` field. If no structure is active, fall back to a flat `recipes/` folder. Never hardcode a location here.

---

## Agent Instructions

You help the user get more value from their personal recipe database. Pages in this domain follow the Recipes template below: each page covers one recipe, with frontmatter (recipe_name, cuisine, meal_type, prep_time, cook_time, servings, dietary_tags, rating) and sections for ingredients, instructions, notes, variations, and pairings.
...
Use this template when creating a new recipe page. This lobe defines the note shape only — it does not pick a folder. The active hippocampus skill decides where it is filed (recorded in `filed_at`); with no structure active, default to `recipes/<recipe-name>.md`.

```markdown
---
file_type: recipes
filed_at: ""
# filed_at: path set by the active hippocampus skill — flat `recipes/` folder if no structure active
recipe_name: ""
cuisine: ""
# cuisine: italian | mexican | japanese | indian | american | mediterranean | other
meal_type: ""
...
# rating: 1–5
dietary_tags: []
# dietary_tags: vegetarian | vegan | gluten-free | dairy-free | nut-free | keto | paleo
source: ""
# source: cookbook name, URL, or "family recipe"
tags: []
---

## Ingredients

<!-- List all ingredients with amounts. Group by component if the recipe has multiple parts (e.g. sauce, filling, dough). -->
...
<!-- Numbered steps. Each step should be one clear action. -->
...
<!-- Substitutions, tips, common mistakes, make-ahead instructions, storage notes. -->

## Variations

<!-- Ways to adapt this recipe: different proteins, dietary swaps, seasonal ingredients. -->

## Pairings

<!-- What goes well with this? Side dishes, drinks, sauces. -->
```
