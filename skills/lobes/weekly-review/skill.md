...
Read this file, then set up a weekly review project in the user's Brains knowledge base using the template and instructions below. Create the project structure, initialize the instructions page, and offer to help with this week's review.

**Filing — important:** This lobe is a *note template only*; it does not choose a folder. Before creating pages, check whether a hippocampus skill (PARA, Johnny.Decimal, Zettelkasten, or Mind Palace) is active. If one is, ask it where each page belongs and record that path in the page's `filed_at` field. If no structure is active, fall back to a flat `weekly-review/` folder. Never hardcode a location here.

---

## Agent Instructions

You help the user reflect on and learn from their weekly reviews. Pages in this domain follow the Weekly Review template below: each page covers one week, with frontmatter (week_start, energy_level, theme) and sections for wins, challenges, lessons, next-week goals, and loose threads.
...
Use this template when creating a new weekly review page. This lobe defines the note shape only — it does not pick a folder. The active hippocampus skill decides where it is filed (recorded in `filed_at`); with no structure active, default to `weekly-review/<YYYY-MM-DD>.md`.

```markdown
---
file_type: weekly-review
filed_at: ""
# filed_at: path set by the active hippocampus skill — flat `weekly-review/` folder if no structure active
week_start: ""
# week_start: ISO date of the Monday, e.g. 2026-05-26
energy_level: 
# energy_level: 1–5 (1 = burned out, 5 = fully energized)
theme: ""
# theme: optional one-word label for the week, e.g. "shipping", "recovery", "deep work"
tags: []
---

## Wins

<!-- What went well? List 3–5 things, big or small. -->
...
<!-- What was hard? What didn't go the way you wanted? Be honest. -->
...
<!-- What did you learn — about your work, yourself, or the world? -->
...
<!-- If you replayed the week, what one or two things would you change? -->

## Next week: goals

<!-- 3–5 concrete goals. Each should be achievable in one week. -->
...
<!-- Things you said you'd do that slipped. Capture them so they don't get lost. -->

-

## Energy note

<!-- Optional. What drained you this week? What gave you energy? -->
```
