...
Use this template when creating a new project tracker page. This lobe defines the note shape only — it does not pick a folder. The active hippocampus skill decides where it is filed (recorded in `filed_at`); with no structure active, default to `project-tracker/<project-name>.md`.

```markdown
---
file_type: project-tracker
filed_at: ""
# filed_at: path set by the active hippocampus skill — flat `project-tracker/` folder if no structure active
project_name: ""
status: planning
