...
Use this template when creating a new book note page. This lobe defines the note shape only — it does not pick a folder. The active hippocampus skill decides where it is filed (recorded in `filed_at`); with no structure active, default to `book-notes/<title-author>.md`.

```markdown
---
file_type: book-notes
filed_at: ""
# filed_at: path set by the active hippocampus skill — flat `book-notes/` folder if no structure active
title: ""
author: ""
year: 
genre: ""
